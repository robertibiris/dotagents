...
Future `setup-agentic-scope` tooling will automate and validate this operation. Until that tooling exists, review both files explicitly before committing.
...
Follow `platform-adapters.md`. Create only the adapters required by the chosen platforms. Adapters reference canonical scope material and must not copy company, client, or project knowledge.
